...
SELECT E.ENAME AS 사원명  -- E.ENAME 칼럼의 별명을 '사원명'으로 부여한다. AS(ALIAS)를 사용할 수 있다.
  FROM EMP E;               -- EMP는 E구나를 지정하고 위로 셀렉트로 올라가서 별명 지정해주는 것
...
-- 3. 동일한 데이터는 한 번만 조회하기(같은 데이터이름이 있으면 중복시키지않고 출력)
--      DISTINCT
SELECT DISTINCT JOB
  FROM EMP;
...
--    1) WILD CARD(만능문자)
--       (1) % : 글자 수 제한 없는 모든 문자 ex ) A% = AI , APP, APPLE 모두가능
--       (2) _ : 한 글자로 제한된 모든 문자 ex ) A_ = AI, AP, AM A로 시작하는 두글자
--    2) 연산자
--       (1) LIKE     : WILD CARD를 포함한다.
--       (2) NOT LIKE : WILD CARD를 포함하지 않는다.
SELECT EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, DEPTNO
  FROM EMP
 WHERE ENAME LIKE 'A%';                 -- WHERE ENAME = 'A%';  WILD CARD를 사용하려면 등호를 사용하지않고 LIKE를 사용해야한다.
/*
    ...
    4. 완료된 작업을 취소할 수 없다.(COMMIT할 필요가 없다. ROLLBACK을 할 수없다.)
    5. 종류
        1) CREATE : 생성
        2) ALTER  : 수정
        3) DROP   : 삭제
...
/*
    테이블 변경(수정)하기 
    1. 칼럼 추가    : ALTER TABLE 테이블명 ADD (추가 할)칼럼명 데이터타입 [제약조건]
    2. 칼럼 삭제    : ALTER TABLE 테이블명 DROP COLUMN 칼럼명
    3. 칼럼 수정    : ALTER TABLE 테이블명 MODIFY (기존에있는)칼럼명 데이터타입 [제약조건]
...
ALTER TABLE BANK_TBL 
    ADD BANK_TEL VARCHAR2(20 BYTE) NOT NULL; -- ( 칼럼 추가 )
    
-- 2. CUSTOMER_TBL 테이블에서 나이(AGE) 칼럼을 삭제하시오.
ALTER TABLE CUSTOMER_TBL
    DROP COLUMN AGE;    -- ( 칼럼 삭제 )
...
ALTER TABLE CUSTOMER_TBL
    MODIFY CUST_NAME VARCHAR2(30 BYTE) NULL;  -- MODIFY CUST_NAME VARCHAR2(30 BYTE);  수정할 때는 제약조건 NULL을 적지 않으면 이전꺼(NOT NULL)를 사용한다는 뜻 !!!!

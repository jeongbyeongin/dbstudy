...
-- 8. 원하는 자릿수로 반올림
--    ROUND(A, [DIGIT]) : 실수 A를 DIGIT 자릿수로 반올림 값을 반환, DIGIT을 정수로 반올림 값을 반환
SELECT
       ROUND(123.456)     -- 정수로 반올림
     , ROUND(123.456, 1)  -- 소수 1자리 남기고 반올림
     , ROUND(123.456, 2)  -- 소수 2자리 남기고 반올림
     , ROUND(123.445, -1) -- 일의 자리에서 반올림
...
--     TRUNC(A, [DIGIT]) : 실수 A를 DIGIT 자릿수로 절사 값을 반환, DIGIT을 정수로 절사한 값을 반환
SELECT
       TRUNC(123.456)     -- 정수로 절사
     , TRUNC(123.456, 1)  -- 소수 1자리 남기고 절사
     , TRUNC(123.456, 2)  -- 소수 2자리 남기고 절사
     , TRUNC(123.445, -1) -- 일의 자리에서 절사

...
SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY
 ...
 WHERE POSITION IN (SELECT POSITION
                     FROM EMPLOYEE_TBL
                    WHERE DEPART = 2);  -- 서브쿼리의 WHERE절에서 사용한 칼럼이 PK나 UNIQUE칼럼이 아니므로 다중 행 서브쿼리이다.
                                        -- 따라서 단일 행 연산자 등호(=)대신 다중 행 연산자 (IN)을 사용해야 한다.
----(SELECT POSITION
--     FROM EMPLOYEE_TBL
--    WHERE DEPART = 2);  위에 다중쿼리이기 때문에 WHERE POSITION IN('부장, '과장'); 으로 바뀐것이기 때문에 1 과장도 나온다.
...
SELECT EMP_NO, NAME, DEPART, POSITION, GENDER, HIRE_DATE, SALARY
 ...
 WHERE DEPART IN (SELECT DEPT_NO
                   FROM DEPARTMENT_TBL
                  WHERE DEPT_NAME = '영업부');  -- 서브쿼리의 WHERE절에서 사용한 DEPART_NAME 칼럼은 PK/UNIQUE가 아니므로 다중 행
...
SELECT EMP_NO, NAME
  FROM EMPLOYEE_TBL
 WHERE ROWID = 'AAAFDwAABAAALG5AAA';

/*
    ROWNUM의 제약 사항
    1. ROWNUM이 1을 포함하는 범위를 조건으로 사용할 수 있다.
    2. ROWNUM이 1을 포함하지 않는 범위는 조건으로 사용할 수 없다.
    3. 모든 ROWNUM을 사용하려먼 ROWNUM에 별명을 지정하고 그 별명을 사용한다,
*/

SELECT EMP_NO, NAME
  FROM EMPLOYEE_TBL
 WHERE ROWNUM = 2;      -- NUM이 1을 포함한 법위가 사용되므로 가능!


SELECT EMP_NO, NAME
  FROM EMPLOYEE_TBL
 WHERE ROWNUM =2;       -- ROWUM이 1을 포함한 번위가 아니므로 불가능!

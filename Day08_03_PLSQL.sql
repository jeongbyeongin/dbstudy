...
--    SELECT절의 INTO를 이용해서 테이블의 데이터를 가져와서 변수에 저장할 수 있습니다.
-- 선언 방법 : 테이블.칼럼명%TYPE
DECLARE
...
BEGIN
    FOR N IN 1..10 LOOP
       SELECT MOD(N, 3)     -- N을 2로나눈 나머지를 MODULAR에 저장해라
         INTO MODULAR
         FROM DUAL;
       IF MODULAR = 0 THEN
            MESSAGE := '3의배수';
...
-- EMPLOYEE_ID가 150인 사원의 SALARY가 15000 이상이면 '고액연봉', 아니면 '보통연봉'
DECLARE
    EMP_ID EMPLOYEES.EMPLOYEE_ID%TYPE;
    SAL EMPLOYEES.SALARY%TYPE;
...
BEGIN
    ...
    DBMS_OUPUT.PUT_LINE('사원번호' || EMP_ID || '인 사원의 커미션은' || MESSAGE || '입니다');
END;
...
/*
    EXIT : 반복문 종료하기
    CONTINUE : LOOP문의 시작부터 다시 실행하기      1 2 3 4 5 CONTINUE; 6 7 8 9 컨티뉴를 만나면 6번으로 가지말고 1번으로 다시돌아가서 실행;    
*/

-- 1부터 정수 값을 누적하시오. 누적 값이 100을 초과하면 그만 동작하고 어디까지 누적했는지 출력하시오.
DECLARE
    N NUMBER;
    TOTAL NUMBER;
BEGIN
...
-- 1부터 3의 배수를 제외한 정수 값을 누적하시오. 누적 값이 100을 초과하면 그만 동작하고 어디까지 누적했는지 출력하시오.
DECLARE
    N NUMBER;
    TOTAL NUMBER;
    MODULAR NUMBER(1);

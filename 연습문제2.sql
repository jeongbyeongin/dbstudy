...
-- INSERT INTO 테이블 VALUES(SYSDATE || '-' ||.NEXTVAL)
...
INSERT INTO ORDER_TBL (ORDER_ID, CUSTOMER_ID, BOOK_ID, AMOUNT, ORDER_DATE) VALUES (TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL, 1000, 7, 1, '20/07/01');
...
INSERT INTO ORDER_TBL (ORDER_ID, CUSTOMER_ID, BOOK_ID, AMOUNT, ORDER_DATE) VALUES (TO_CHAR(SYSDATE, 'YYMMDD-') || ORDER_SEQ.NEXTVAL, 1000, 2, 2, '20/07/07');
...
SELECT O.ORDER_ID, C.CUSTOMER_NAME, B.BOOK_NAME, B.PRICE * O.AMOUNT, O.ORDER_DATE
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
...
SELECT C.CUSTOMER_NAME, SUM(B.PRICE * O.AMOUNT) AS 총구매액
  FROM CUSTOMER_TBL C INNER JOIN ORDER_TBL O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK_TBL B
    ON O.BOOK_ID = B.BOOK_ID
 GROUP BY C.CUSTOMER_NAME, C.CUSTOMER_ID;
